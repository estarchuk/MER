...
import mer.mod.MER;
import mer.mod.MERTab;
import mer.mod.Reference;
import mer.mod.entities.TileTrumpDim;
...
public class BlockTrumpPortal extends BlockPortal {
    ...
    public BlockTrumpPortal() {
        super();
        this.setDefaultState(this.blockState.getBaseState().withProperty(AXIS, EnumFacing.Axis.X));
        this.setTickRandomly(true);
        setUnlocalizedName(Reference.ModBlocks.TRUMPPORTAL.getUnlocalizedName());
        setRegistryName(Reference.ModBlocks.TRUMPPORTAL.getRegistryName());
    ...
    public boolean trySpawnPortal(World worldIn, BlockPos pos) {
        BlockTrumpPortal.Size blockportal$size = new BlockTrumpPortal.Size(worldIn, pos, EnumFacing.Axis.X);

        if (blockportal$size.isValid() && blockportal$size.portalBlockCount == 0) {
            blockportal$size.placePortalBlocks();
            return true;
        } else {
            BlockTrumpPortal.Size blockportal$size1 = new BlockTrumpPortal.Size(worldIn, pos, EnumFacing.Axis.Z);
    ...
    public BlockPattern.PatternHelper createPatternHelper(World worldIn, BlockPos p_181089_2_) {
        ...
        BlockTrumpPortal.Size blockportal$size = new BlockTrumpPortal.Size(worldIn, p_181089_2_, EnumFacing.Axis.X);
        LoadingCache<BlockPos, BlockWorldState> loadingcache = BlockPattern.createLoadingCache(worldIn, true);

        if (!blockportal$size.isValid()) {
            enumfacing$axis = EnumFacing.Axis.X;
            blockportal$size = new BlockTrumpPortal.Size(worldIn, p_181089_2_, EnumFacing.Axis.Z);
        }
    ...
    public static class Size {
        ...
        protected int calculatePortalHeight() {
            ...
            for (this.height = 0; this.height < 21; ++this.height) {
                for (int i = 0; i < this.width; ++i) {
                    ...
                    if (block == ModBlocks.trumpportal) {
        ...
        protected boolean isEmptyBlock(Block blockIn) {
            return blockIn.getMaterial(null) == Material.AIR || blockIn == Blocks.FIRE || blockIn == ModBlocks.trumpportal;
        }
        ...
        public void placePortalBlocks() {
            for (int i = 0; i < this.width; ++i) {
                ...
                for (int j = 0; j < this.height; ++j) {
                    this.world.setBlockState(blockpos.up(j), ModBlocks.trumpportal.getDefaultState().withProperty(BlockTrumpPortal.AXIS, this.axis), 2);
                }
            }
        }
    }
